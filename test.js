...
function pixelsToHex(pos, width, height) {
  ...
  // yHex = yHex < 0.25 ? yHex - 0.25  : yHex + 0.25 * Math.floor(pos.y / (height * 0.75) - 0.25);
  yHex += 0.25 * Math.floor(pos.y / (height * 0.75) - 0.25);
  console.log(yHex, Math.floor(pos.y / (height * 0.75)));
  const xHex = pos.x / width;
  const x = xHex;
  const y = yHex;
...
test('pixelsToHex', t => {
  let width = 86.60254037844386;
  let side = width / Math.sqrt(3);
  let height = 2 * side;
  let radius = width / 2;
  let hexHeight = side / 2;

  let pos = { x: 0, y: 0 };
  let res = pixelsToHex(pos, width, height);
  t.is(res.x, 0);
  t.is(res.y, -0.25);

  pos = { x: 0, y: 25 };
  res = pixelsToHex(pos, width, height);
  t.is(res.x, 0);
  t.is(res.y, 0.25);
  ...
  t.is(res.x, 0);
  t.is(res.y, 2.25);
  // t.deepEqual({ x: res.x , y: res.y }, { x: 0, y: 1 })
});